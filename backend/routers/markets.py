...
EIEL_TABLE_NAME = "lonja_merc_feria"
ORDER_FIELD = "orden_lmf"
...
async def delete_by_gid(
    ...
    query = sql.text(f"""
        delete from eiel.lonja_merc_feria
        where gid = :gid;
    """)
    values = {
        "gid": gid
...
async def insert_equipment(
    ...
    equipment.orden_lmf = result.get("order_field")
...
async def insert_equipment_in_db(equipment, db_postgres):
    ...
    query = sql.text(f"""
        INSERT INTO eiel.lonja_merc_feria(
            fase, clave, prov, mun, ent, poblamiento, orden_lmf, nombre, tipo_lonj, titular, gestion,  
        ...
        RETURNING eiel.lonja_merc_feria.gid;
    """)
    values = {
        "fase": equipment.fase,
