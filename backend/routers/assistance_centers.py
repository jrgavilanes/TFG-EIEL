...
EIEL_TABLE_NAME = "centro_asistencial"
ORDER_FIELD = "orden_casis"
NOMECALLES_TABLES_NAMES = ["ssociales", "inmigrantes"]
TIPO_EQUIPAMIENTO = "AS"

router = APIRouter(
    prefix='/api/assistance-centers',
    tags=['/api/assistance-centers']
)


class AssistanceCenterCreateRequest(BaseModel):
    fase: str
    clave: str
    prov: str
    mun: str
...
class AssistanceCenterUpdateRequest(BaseModel):
    gid: int
    nombre: str
    tipo_casis: Optional[str]
    titular: Optional[str]
...
async def delete_by_gid(
    ...
    query = sql.text(f"""
        delete from eiel.centro_asistencial
        where gid = :gid;
    """)
    values = {
        "gid": gid
...
async def update_by_gid(
        user: user_dependency,
        gid: int,
        equipment: AssistanceCenterUpdateRequest,
        db_postgres: db_dependency_postgres
):
    ...
    query = sql.text(f"""
        update eiel.centro_asistencial
        set nombre = :nombre,
            tipo_casis = :tipo_casis,
            titular = :titular,
            gestion = :gestion,
            plazas = :plazas,
            s_cubi = :s_cubi,
    ...
    values = {
        ...
        "tipo_casis": equipment.tipo_casis,
        "titular": equipment.titular,
        "gestion": equipment.gestion,
        "plazas": equipment.plazas,
...
async def insert_equipment(
        user: user_dependency,
        db_postgres: db_dependency_postgres,
        equipment: AssistanceCenterCreateRequest
):
    if user.get("role") not in ["admin", "operator"]:
    ...
    equipment.orden_casis = result.get("order_field")
...
async def insert_equipment_in_db(equipment, db_postgres):
    ...
    query = sql.text(f"""
        INSERT INTO eiel.centro_asistencial(
            fase, clave, prov, mun, ent, poblamiento, orden_casis, nombre, tipo_casis, titular, gestion, 
            plazas, s_cubi, s_aire, s_sola, acceso_s_ruedas, estado, cod, borrado, 
            geom            
        )
        VALUES (
            :fase, :clave, :prov, :mun, :ent, :poblamiento, :orden_casis, :nombre, :tipo_casis, :titular, :gestion, 
        ...
        RETURNING eiel.centro_asistencial.gid;
    """)
    values = {
        "fase": equipment.fase,
        ...
        "orden_casis": equipment.orden_casis,
        "nombre": equipment.nombre,
        "tipo_casis": equipment.tipo_casis,
        "titular": equipment.titular,
        "gestion": equipment.gestion,
