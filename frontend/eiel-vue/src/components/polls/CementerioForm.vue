...
import { aa_estado, aa_acceso_s_ruedas } from '@/composables/dominioComposable.js';
import { cementerio_ampliacion, cementerio_capilla, cementerio_crematorio, cementerio_deposito, cementerio_titular } from '@/composables/cementerioComposable.js';
...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/cemeteries/${gid}`, {
            method: 'GET',
            headers: {
        ...
        formData.distancia = result.distancia === null ? null : `${result.distancia}`
        formData.acceso = result.acceso
        formData.capilla = result.capilla
        formData.deposito = result.deposito
        formData.ampliacion = result.ampliacion
        formData.saturacion = result.saturacion === null ? null : `${result.saturacion}`
        formData.superficie = result.superficie === null ? null : `${result.superficie}`
        formData.acceso_s_ruedas = result.acceso_s_ruedas
        formData.crematorio = result.crematorio
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        fetch('/api/cemeteries/', {
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_cementerios').setData(data);
                                mapStore.map.setLayoutProperty("cementerios", 'visibility', 'none')
                                mapStore.map.setLayoutProperty("cementerios", 'visibility', 'visible')
...
const postInfo = () => {
    ...
    if (formData.gid) {
        ...
        const payload = {
            ...
            orden_cement: formData.orden_cement,
            clave: props.tipo_equipamiento,
            nombre: formData.nombre,
            cod: formData.cod,
            titular: formData.titular || '',
        ...
        fetch('/api/cemeteries/', {
            ...
            .then(data => {
                ...
                Swal.fire({
                    ...
                    mapStore.listaEquipamientoMunicipales.cementerios = true;
                        ...
                        if (mapStore.showEquipmentForm) {
                            ...
                            fetch('/api/cemeteries/', {
                                ...
                                .then(data => {
                                    mapStore.map.getSource('mi_cementerios').setData(data);
                                    try {
                                        mapStore.map.moveLayer('cementerios', 'mi_cementerios')
                                    } catch (error) {
...
//             alert('El acceso a la ubicación fue denegado.');
//             isLoading.value = false;
//         }
//         // result.onchange = function () {
//         //     // alert('El permiso de ubicación cambió a ' + result.state);
//         // };
//     });
...
const isValidForm = () => {
    return formData.nombre &&
        formData.titular &&
        !isNaN(formData.distancia) && formData.distancia &&
        formData.acceso &&
        formData.capilla &&
        formData.deposito &&
        formData.ampliacion &&
        !isNaN(formData.saturacion) && formData.saturacion &&
        !isNaN(formData.superficie) && formData.superficie &&
        ...
        <form @submit.prevent="postInfo()" class="max-w-3xl mx-auto mt-28 select-none overscroll-none">
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <p v-if="!formData.nombre" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.titular" id="titular"
                        ...
                        <option v-for="estado in cementerio_titular" :key="estado.valor" :value="estado.valor">{{
                estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.distancia" type="tel" name="distancia" id="distancia"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        pattern="^\d*(\.\d+)?$" placeholder="" required />
                    <p v-if="!formData.distancia || isNaN(Number(formData.distancia))"
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="acceso" class="block mb-2 text-sm font-medium text-gray-900 ">Acceso</label>
                    <select v-model="formData.acceso" id="acceso"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                    ...
                    <p v-if="!formData.acceso" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="capilla" class="block mb-2 text-sm font-medium text-gray-900 ">Capilla</label>
                    <select v-model="formData.capilla" id="capilla"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in cementerio_capilla" :key="estado.valor" :value="estado.valor">{{
                estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.capilla" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="deposito" class="block mb-2 text-sm font-medium text-gray-900 ">Depósito</label>
                    <select v-model="formData.deposito" id="deposito"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in cementerio_deposito" :key="estado.valor" :value="estado.valor">{{
                estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.deposito" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="ampliacion" class="block mb-2 text-sm font-medium text-gray-900 ">Ampliación</label>
                    <select v-model="formData.ampliacion" id="ampliacion"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in cementerio_ampliacion" :key="estado.valor" :value="estado.valor">{{
                estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.ampliacion" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.saturacion" type="tel" name="saturacion" id="saturacion"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.saturacion || isNaN(Number(formData.saturacion))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.superficie" type="tel" name="superficie" id="superficie"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.superficie || isNaN(Number(formData.superficie))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="acceso_s_ruedas" class="block mb-2 text-sm font-medium text-gray-900 ">Acceso
                        silla de ruedas</label>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="crematorio" class="block mb-2 text-sm font-medium text-gray-900 ">Crematorio</label>
                    <select v-model="formData.crematorio" id="crematorio"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in cementerio_crematorio" :key="estado.valor" :value="estado.valor">{{
                estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.crematorio" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
            </div>
            <!-- botonera inferior -->
