...
import { aa_gestion_equip, aa_titular_equip, cent_cultural_tipo_centro, cent_cultural_usos_uso } from '@/composables/culturalCenterComposable.js';
...
const enable_nomecalles_layers = (layers) => {
    layers.forEach(layer => {
        mapStore.listaCentrosCulturales[layer] = true
    })
}

const move_nomecalles_layers_behind_mine = (layers) => {
    layers.forEach(layer => {
        mapStore.map.moveLayer(layer, 'mi_centros_culturales')
    })
}
...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/cultural-centers/${gid}`, {
            method: 'GET',
            headers: {
        ...
        formData.tipo_centro = result.tipo_centro
...
const deleteEquipment = (gid) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`/api/cultural-centers/${gid}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + userStore.user.token,
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        fetch('/api/cultural-centers/', {
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_centros_culturales').setData(data);
...
const postInfo = () => {
    ...
    if (formData.gid) {
        ...
        const payload = {
            ...
            tipo_centro: formData.tipo_centro || '',
        ...
        fetch(`/api/cultural-centers/${formData.gid}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + userStore.user.token,
        ...
        const payload = {
            ...
            orden_centro: formData.orden_centro,
            nombre: formData.nombre,
            tipo_centro: formData.tipo_centro || '',
            titular: formData.titular || '',
            gestion: formData.gestion || '',
            ...
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <p v-if="!formData.nombre || formData.nombre.length > 50" class="text-red-500 text-xs mt-2">
                        Campo obligatorio de 50 caracteres máximo
                    </p>
                    <label for="nombre"
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="tipo_centro" class="block mb-2 text-sm font-medium text-gray-900 ">Tipo</label>
                    <select v-model="formData.tipo_centro" id="tipo_centro"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in cent_cultural_tipo_centro" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.tipo_centro" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="gestion" class="block mb-2 text-sm font-medium text-gray-900 ">gestion</label>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <div class="grid grid-cols-1 gap-2">
                        <div v-for="uso in cent_cultural_usos_uso" :key="uso.valor">
                            ...
                            <div v-if="formData.usos[uso.valor]" class="mb-4 px-4">
                                <div>
                                    <p class="mx-4 py-2.5 text-xs text-zinc-700">
                                        Superfice cubierta de {{ uso.descripcion }}
                                    </p>
                                    ...
                                    <p v-if="!formData.usos_s_cubi[uso.valor]" class="text-red-500 text-xs mx-4 mt-2">
