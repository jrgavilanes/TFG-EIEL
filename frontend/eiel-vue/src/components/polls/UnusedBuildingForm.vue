...
import { edific_pub_sin_uso_titular, edific_pub_sin_uso_usoant } from '@/composables/unUsedBuildingComposable.js';
...
const enable_nomecalles_layers = (layers) => {
    layers.forEach(layer => {
        mapStore.listaEquipamientoMunicipales[layer] = true
    })
}

const move_nomecalles_layers_behind_mine = (layers) => {
    layers.forEach(layer => {
        mapStore.map.moveLayer(layer, 'mi_edificios_sin_uso')
    })
}
...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/unused-public-buildings/${gid}`, {
            method: 'GET',
            headers: {
...
const deleteEquipment = (gid) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`/api/unused-public-buildings/${gid}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + userStore.user.token,
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        // refresh layer map
                        fetch('/api/unused-public-buildings/', {
                            headers: {
                                'Content-Type': 'application/json',
                                'Authorization': 'Bearer ' + userStore.user.token,
                            }
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_edificios_sin_uso').setData(data);
...
const postInfo = () => {
    ...
    if (formData.gid) {
        ...
        fetch(`/api/unused-public-buildings/${formData.gid}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + userStore.user.token,
        ...
        const payload = {
            ...
            orden_edific: formData.orden_edific,
            nombre: formData.nombre,
            usoant: formData.usoant || '',
            titular: formData.titular || '',
            s_cubi: formData.s_cubi || null,
    ...
    <div
        ...
        <div class="flex justify-start">
            ...
            <p v-if="!formData.gid" class="font-bold text-xl md:text-2xl my-2">
                Alta equipamiento (Edificio público sin uso)
            </p>
            <p v-if="formData.gid" class="font-bold text-xl md:text-2xl my-2 text-ellipsis">
                Editar equipamiento (Edificio público sin uso)
            </p>
        </div>
            ...
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.titular" id="titular"
                        ...
                        <option v-for="estado in edific_pub_sin_uso_titular" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="tipo" class="block mb-2 text-sm font-medium text-gray-900 ">Uso anterior</label>
                    <select v-model="formData.usoant" id="tipo"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in edific_pub_sin_uso_usoant" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.usoant" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
