...
import { centro_sanitario_tipo_csan, centro_sanitario_gestion, centro_sanitario_titular } from '@/composables/medicalCenterComposable.js';
...
const enable_nomecalles_layers = (layers) => {
    layers.forEach(layer => {
        mapStore.listaCentrosSanitarios[layer] = true
    })
}

const move_nomecalles_layers_behind_mine = (layers) => {
    layers.forEach(layer => {
        mapStore.map.moveLayer(layer, 'mi_centros_sanitarios')
    })
}
...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/medical-centers/${gid}`, {
            method: 'GET',
            headers: {
        ...
        formData.tipo_csan = result.tipo_csan
        ...
        formData.uci = result.uci
        formData.camas = result.camas
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        fetch('/api/medical-centers/', {
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_centros_sanitarios').setData(data);
...
const postInfo = () => {
    ...
    formData.camas = formData.camas ? Number(formData.camas) : null;
    ...
    if (formData.gid) {
        ...
        const payload = {
            ...
            uci: formData.uci || '',
            camas: formData.camas || null,
            acceso_s_ruedas: formData.acceso_s_ruedas || '',
            estado: formData.estado || ''
        }


        fetch(`/api/medical-centers/${formData.gid}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + userStore.user.token,
        ...
        const payload = {
            ...
            orden_csan: formData.orden_csan,
            nombre: formData.nombre,
            tipo_csan: formData.tipo_csan || '',
            titular: formData.titular || '',
            gestion: formData.gestion || '',
        ...
        <form @submit.prevent="postInfo()" class="max-w-3xl mx-auto mt-28 select-none overscroll-none">
            ...
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.tipo_csan" id="tipo"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in centro_sanitario_tipo_csan" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.tipo_csan" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.titular" id="titular"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in centro_sanitario_titular" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.gestion" id="gestion"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in centro_sanitario_gestion" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="uci" class="block mb-2 text-sm font-medium text-gray-900 ">UCI</label>
                    <select v-model="formData.uci" id="uci"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                    ...
                    <p v-if="!formData.uci" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.camas" type="tel" name="camas" id="camas"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
