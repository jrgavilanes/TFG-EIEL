...
import { aa_estado, aa_acceso_s_ruedas, aa_fase, aa_sino, aa_titular_equip, aa_gestion_equip } from '@/composables/dominioComposable.js';
import { matadero_clase_mat } from '@/composables/slaughterhouseComposable.js';
...
const enable_nomecalles_layers = (layers) => {
    layers.forEach(layer => {
        mapStore.listaLonjasMercadosFerias[layer] = true
    })
}

const move_nomecalles_layers_behind_mine = (layers) => {
    layers.forEach(layer => {
        mapStore.map.moveLayer(layer, 'mi_mataderos')
    })
}
...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/slaughterhouses/${gid}`, {
            method: 'GET',
            headers: {
        ...
        formData.clase_mat = result.clase_mat
        ...
        formData.capacidad = result.capacidad === null ? null : `${result.capacidad}`
        formData.utilizacio = result.utilizacio === null ? null : `${result.utilizacio}`
        formData.tunel = result.tunel
        formData.bovino = result.bovino
        formData.ovino = result.ovino
        formData.porcino = result.porcino
        formData.otros = result.otros
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        fetch('/api/slaughterhouses/', {
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_mataderos').setData(data);
...
const postInfo = () => {
    ...
    formData.capacidad = formData.capacidad ? Number(formData.capacidad) : null;
    formData.utilizacio = formData.utilizacio ? Number(formData.utilizacio) : null;
    ...
    if (formData.gid) {
        ...
        const payload = {
            ...
            utilizacio: formData.utilizacio || null,
            tunel: formData.tunel || '',
            bovino: formData.bovino || '',
            ovino: formData.ovino || '',
            porcino: formData.porcino || '',
            otros: formData.otros || '',
        }


        fetch(`/api/slaughterhouses/${formData.gid}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + userStore.user.token,
        ...
        const payload = {
            ...
            orden_matad: formData.orden_matad,
            nombre: formData.nombre,
            clase_mat: formData.clase_mat || '',
            titular: formData.titular || '',
            gestion: formData.gestion || '',
    ...
    <div v-if="!isLoading">
        <form @submit.prevent="postInfo()" class="max-w-3xl mx-auto mt-28 select-none overscroll-none">
            ...
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="tipo" class="block mb-2 text-sm font-medium text-gray-900 ">Clase matadero</label>
                    <select v-model="formData.clase_mat" id="tipo"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in matadero_clase_mat" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.clase_mat" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.titular" id="titular"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_titular_equip" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.gestion" id="gestion"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_gestion_equip" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.capacidad" type="tel" name="capacidad" id="capacidad"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.utilizacio" type="tel" name="utilizacio" id="utilizacio"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" pattern="\d*" />
                    <label for="utilizacio"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        Utilización
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="tunel" class="block mb-2 text-sm font-medium text-gray-900 ">
                        Túnel
                    </label>
                    <select v-model="formData.tunel" id="tunel"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                    ...
                    <p v-if="!formData.tunel" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="bovino" class="block mb-2 text-sm font-medium text-gray-900 ">
                        Bobino
                    </label>
                    <select v-model="formData.bovino" id="bovino"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_sino" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                    ...
                    <p v-if="!formData.bovino" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="ovino" class="block mb-2 text-sm font-medium text-gray-900 ">
                        Ovino
                    </label>
                    <select v-model="formData.ovino" id="ovino"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_sino" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                    ...
                    <p v-if="!formData.ovino" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="porcino" class="block mb-2 text-sm font-medium text-gray-900 ">
                        Porcino
                    </label>
                    <select v-model="formData.porcino" id="porcino"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_sino" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                    ...
                    <p v-if="!formData.porcino" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="otros" class="block mb-2 text-sm font-medium text-gray-900 ">
                        Otros
                    </label>
