...
import { aa_estado, aa_acceso_s_ruedas, aa_fase, aa_titular_equip, aa_gestion_equip, } from '@/composables/dominioComposable.js';
import { centro_asistencia_tipo_casis } from '@/composables/assistanceCenterComposable.js';
...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/assistance-centers/${gid}`, {
            method: 'GET',
            headers: {
...
const deleteEquipment = (gid) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`/api/assistance-centers/${gid}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + userStore.user.token,
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        fetch('/api/assistance-centers/', {
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_centros_asistencia').setData(data);
                                mapStore.map.setLayoutProperty("ssociales", 'visibility', 'none')
                                mapStore.map.setLayoutProperty("ssociales", 'visibility', 'visible')
...
const postInfo = () => {
    ...
    formData.plazas = formData.plazas ? Number(formData.plazas) : null;
    ...
    if (formData.gid) {
        ...
        const payload = {
            ...
            tipo_casis: formData.tipo_casis || '',
            titular: formData.titular || '',
            gestion: formData.gestion || '',
            plazas: formData.plazas || null,
        ...
        fetch(`/api/assistance-centers/${formData.gid}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + userStore.user.token,
        ...
        const payload = {
            ...
            orden_casis: formData.orden_casis,
            nombre: formData.nombre,
            tipo_casis: formData.tipo_casis || '',
            titular: formData.titular || '',
            gestion: formData.gestion || '',
        ...
        fetch('/api/assistance-centers/', {
            ...
            .then(data => {
                ...
                Swal.fire({
                    ...
                    mapStore.listaCentrosAsistencia.ssociales = true;
                    mapStore.listaCentrosAsistencia.inmigrantes = true;
                        ...
                        if (mapStore.showEquipmentForm) {
                            ...
                            fetch('/api/assistance-centers/', {
                                ...
                                .then(data => {
                                    mapStore.map.getSource('mi_centros_asistencia').setData(data);
                                    try {
                                        mapStore.map.moveLayer('ssociales', 'mi_centros_asistencia')
                                    } catch (error) {
                        ...
                        if (mapStore.showEquipmentForm) {
                            fetch('/api/assistance-centers/', {
                                ...
                                .then(data => {
                                    ...
                                    try {
                                        mapStore.map.moveLayer('ssociales', 'mi_centros_asistencia')
                                        mapStore.map.moveLayer('inmigrantes', 'mi_centros_asistencia')
                                    } catch (error) {
                                        console.log('error al mover capa', error)
                                    }
                                    window.scrollTo({
    ...
    <div
        ...
        <div class="flex justify-start">
            ...
            <p v-if="!formData.gid" class="font-bold text-xl md:text-2xl my-2">
                Alta equipamiento (Centro de asistencia)
            </p>
            <p v-if="formData.gid" class="font-bold text-xl md:text-2xl my-2 text-ellipsis">
                Editar equipamiento (Centro de asistencia)
            </p>
        </div>
        ...
        <form @submit.prevent="postInfo()" class="max-w-3xl mx-auto mt-28 select-none overscroll-none">
            ...
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.tipo_casis" id="tipo"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in centro_asistencia_tipo_casis" :key="estado.valor"
                            :value="estado.valor">{{
                estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.tipo_casis" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.plazas" type="tel" name="plazas" id="plazas"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
