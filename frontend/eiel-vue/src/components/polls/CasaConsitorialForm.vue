...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/townhalls/${gid}`, {
            method: 'GET',
            headers: {
        ...
        formData.tipo = result.tipo
        formData.cod = result.cod
        formData.titular = result.titular
        formData.tenencia = result.tenencia
        ...
        formData.usos = result.usos
        formData.usos_s_cubi = result.usos_s_cubi
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        fetch('/api/townhalls/', {
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_casas_consistoriales').setData(data);
                                mapStore.map.setLayoutProperty("casas_consistoriales", 'visibility', 'none')
                                mapStore.map.setLayoutProperty("casas_consistoriales", 'visibility', 'visible')
...
const postInfo = () => {
    ...
    // Sanitiza los usos a enviar
    let usos_activos = 0;
    for (let key in formData.usos) {
        if (formData.usos[key] === false) {
            delete formData.usos_s_cubi[key];
        } else {
            formData.usos_s_cubi[key] = formData.usos_s_cubi[key] || 0;
            usos_activos++;
        }
    }
    ...
    if (formData.gid) {
        ...
        const payload = {
            ...
            usos: formData.usos,
            usos_s_cubi: formData.usos_s_cubi
        }


        fetch(`/api/townhalls/${formData.gid}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + userStore.user.token,
        ...
        const payload = {
            ...
            orden_casa: formData.orden_casa,
            nombre: formData.nombre,
            tipo: formData.tipo || '',
            titular: formData.titular || '',
            tenencia: formData.tenencia || '',
                        ...
                        if (mapStore.showEquipmentForm) {
                            ...
                            fetch('/api/townhalls/', {
                                ...
                                .then(data => {
                                    mapStore.map.getSource('mi_casas_consistoriales').setData(data);
                                    try {
                                        mapStore.map.moveLayer('casas_consistoriales', 'mi_casas_consistoriales')
                                    } catch (error) {
        ...
        <form @submit.prevent="postInfo()" class="max-w-3xl mx-auto mt-28 select-none overscroll-none">
            ...
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.tipo" id="tipo"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in casa_consistorial_tipo" :key="estado.valor" :value="estado.valor">{{
                estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.tipo" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.titular" id="titular"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in casa_consistorial_titular" :key="estado.valor" :value="estado.valor">{{
                estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="tenencia" class="block mb-2 text-sm font-medium text-gray-900 ">Tenencia</label>
                    <select v-model="formData.tenencia" id="tenencia"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in casa_consistorial_tenencia" :key="estado.valor" :value="estado.valor">
                            {{
                estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.tenencia" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <p class="text-xl">Usos</p>
                    <hr class="mb-4 mt-2">
                    <div class="grid grid-cols-1 gap-2">
                        <div v-for="uso in casa_consistorial_uso" :key="uso.valor">
                            <input v-model="formData.usos[uso.valor]" type="checkbox" :id="uso.valor" :value="uso.valor"
                                class="peer mx-4" />
                            <label :for="uso.valor"
                            ...
                            <div v-if="formData.usos[uso.valor]" class="mb-4 px-4">
                                <p class="mx-4 py-2.5 text-xs text-zinc-700">
                                    Superficie cubierta de {{ uso.descripcion }}
                                </p>
                                <input v-model="formData.usos_s_cubi[uso.valor]" type="tel"
                                ...
                                <p v-if="!formData.usos_s_cubi[uso.valor]?.toString()"
                                    class="text-red-500 text-xs mx-4 mt-2">Campo
                                    numérico
                                    entero obligatorio</p>
                            </div>
