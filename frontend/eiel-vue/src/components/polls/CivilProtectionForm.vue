...
import { aa_estado, aa_acceso_s_ruedas, aa_fase, aa_sino } from '@/composables/dominioComposable.js';
import { proteccion_civil_ambito, proteccion_civil_gestion, proteccion_civil_tipo_pciv, proteccion_civil_titular } from '@/composables/civilProtectionComposable.js';
...
const enable_nomecalles_layers = (layers) => {
    layers.forEach(layer => {
        mapStore.listaBomberos[layer] = true
    })
}

const move_nomecalles_layers_behind_mine = (layers) => {
    layers.forEach(layer => {
        mapStore.map.moveLayer(layer, 'mi_proteccion_civil')
    })
}
...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/civil-protections/${gid}`, {
            method: 'GET',
            headers: {
        ...
        formData.tipo_pciv = result.tipo_pciv
        formData.titular = result.titular
        formData.gestion = result.gestion
        formData.ambito = result.ambito
        formData.plan_profe = result.plan_profe === null ? null : `${result.plan_profe}`
        formData.plan_volun = result.plan_volun === null ? null : `${result.plan_volun}`
        ...
        formData.vehic_incendio = result.vehic_incendio === null ? null : `${result.vehic_incendio}`
        formData.vehic_rescate = result.vehic_rescate === null ? null : `${result.vehic_rescate}`
        formData.ambulancia = result.ambulancia === null ? null : `${result.ambulancia}`
        formData.medios_aereos = result.medios_aereos === null ? null : `${result.medios_aereos}`
        formData.otros_vehi = result.otros_vehi === null ? null : `${result.otros_vehi}`
        formData.quitanieve = result.quitanieve === null ? null : `${result.quitanieve}`
        formData.detec_ince = result.detec_ince === null ? null : `${result.detec_ince}`
        formData.otros = result.otros === null ? null : `${result.otros}`
...
const deleteEquipment = (gid) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`/api/civil-protections/${gid}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + userStore.user.token,
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        fetch('/api/civil-protections/', {
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_proteccion_civil').setData(data);
...
const postInfo = () => {
    ...
    formData.nombre = formData.nombre.substring(0, 40);
    ...
    if (formData.gid) {
        ...
        const payload = {
            ...
            tipo_pciv: formData.tipo_pciv || '',
            titular: formData.titular || '',
            gestion: formData.gestion || '',
            ambito: formData.ambito || '',
            plan_profe: formData.plan_profe || null,
            plan_volun: formData.plan_volun || null,
            ...
            vehic_incendio: formData.vehic_incendio || null,
            vehic_rescate: formData.vehic_rescate || null,
            ambulancia: formData.ambulancia || null,
            medios_aereos: formData.medios_aereos || null,
            otros_vehi: formData.otros_vehi || null,
            quitanieve: formData.quitanieve || null,
            detec_ince: formData.detec_ince || null,
            otros: formData.otros || null,
        }


        fetch(`/api/civil-protections/${formData.gid}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + userStore.user.token,
        ...
        const payload = {
            ...
            orden_prot: formData.orden_prot,
            nombre: formData.nombre,
            tipo_pciv: formData.tipo_pciv || '',
            titular: formData.titular || '',
            gestion: formData.gestion || '',
...
const isValidForm = () => {
    ...
    return formData.nombre && formData.nombre.length <= 50 &&
        formData.tipo_pciv &&
        formData.titular &&
        formData.gestion &&
        formData.ambito &&
        !isNaN(formData.plan_profe) && formData.plan_profe &&
        !isNaN(formData.plan_volun) && formData.plan_volun &&
        ...
        formData.estado &&
        !isNaN(formData.vehic_incendio) && formData.vehic_incendio &&
        !isNaN(formData.vehic_rescate) && formData.vehic_rescate &&
        !isNaN(formData.ambulancia) && formData.ambulancia &&
        !isNaN(formData.medios_aereos) && formData.medios_aereos &&
        !isNaN(formData.otros_vehi) && formData.otros_vehi &&
        !isNaN(formData.quitanieve) && formData.quitanieve &&
        !isNaN(formData.detec_ince) && formData.detec_ince &&
        !isNaN(formData.otros) && formData.otros
        ...
        <form @submit.prevent="postInfo()" class="max-w-3xl mx-auto mt-28 select-none overscroll-none">
            ...
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <p v-if="!formData.nombre || formData.nombre.length > 50" class="text-red-500 text-xs mt-2">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="tipo_pciv" class="block mb-2 text-sm font-medium text-gray-900 ">Tipo</label>
                    <select v-model="formData.tipo_pciv" id="tipo_pciv"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in proteccion_civil_tipo_pciv" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.tipo_pciv" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.titular" id="titular"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in proteccion_civil_titular" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.gestion" id="gestion"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in proteccion_civil_gestion" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.ambito" id="ambito"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in proteccion_civil_ambito" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.plan_profe" type="tel" name="plan_profe" id="plan_profe"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.plan_profe || isNaN(Number(formData.plan_profe))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="plan_profe"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        plan_profe
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.plan_volun" type="tel" name="plan_volun" id="plan_volun"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.plan_volun || isNaN(Number(formData.plan_volun))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="plan_volun"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        plan_volun
                    </label>
                </div>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <label for="s_aire"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        Superficie aire libre (m2)</label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.vehic_incendio" type="tel" name="vehic_incendio" id="vehic_incendio"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.vehic_incendio || isNaN(Number(formData.vehic_incendio))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="vehic_incendio"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        vehic_incendio
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.vehic_rescate" type="tel" name="vehic_rescate" id="vehic_rescate"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.vehic_rescate || isNaN(Number(formData.vehic_rescate))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="vehic_rescate"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        vehic_rescate
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.ambulancia" type="tel" name="ambulancia" id="ambulancia"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.ambulancia || isNaN(Number(formData.ambulancia))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="ambulancia"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        ambulancia
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.medios_aereos" type="tel" name="medios_aereos" id="medios_aereos"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.medios_aereos || isNaN(Number(formData.medios_aereos))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="medios_aereos"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        medios_aereos
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.otros_vehi" type="tel" name="otros_vehi" id="otros_vehi"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.otros_vehi || isNaN(Number(formData.otros_vehi))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="otros_vehi"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        otros_vehi
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.quitanieve" type="tel" name="quitanieve" id="quitanieve"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.quitanieve || isNaN(Number(formData.quitanieve))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="quitanieve"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        quitanieve
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.detec_ince" type="tel" name="detec_ince" id="detec_ince"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.detec_ince || isNaN(Number(formData.detec_ince))"
                        class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="detec_ince"
                        class="peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600  peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">
                        detec_ince
                    </label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <input v-model="formData.otros" type="tel" name="otros" id="otros"
                        class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                        placeholder="" required pattern="\d*" />
                    <p v-if="!formData.otros || isNaN(Number(formData.otros))" class="text-red-500 text-xs mt-2">
                        Campo
                        numérico entero
                        obligatorio</p>
                    <label for="otros"
