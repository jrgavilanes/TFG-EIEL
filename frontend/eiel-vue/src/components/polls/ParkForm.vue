...
import { aa_estado, aa_acceso_s_ruedas, aa_fase, aa_titular_equip, aa_gestion_equip, aa_sino } from '@/composables/dominioComposable.js';
import { parque_tipo_parq } from '@/composables/parkComposable.js';
...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/parks/${gid}`, {
            method: 'GET',
            headers: {
        ...
        formData.tipo_parq = result.tipo_parq
        ...
        formData.agua = result.agua
        formData.saneamiento = result.saneamiento
        formData.electricidad = result.electricidad
        formData.comedor = result.comedor
        formData.juegos_inf = result.juegos_inf
        formData.otras = result.otras
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        fetch('/api/parks/', {
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_parques').setData(data);
                                mapStore.map.setLayoutProperty("parques", 'visibility', 'none')
                                mapStore.map.setLayoutProperty("parques", 'visibility', 'visible')
...
const postInfo = () => {
    ...
    if (formData.gid) {
        ...
        const payload = {
            ...
            agua: formData.agua || '',
            saneamiento: formData.saneamiento || '',
            electricidad: formData.electricidad || '',
            comedor: formData.comedor || '',
            juegos_inf: formData.juegos_inf || '',
        ...
        const payload = {
            ...
            orden_parq: formData.orden_parq,
            nombre: formData.nombre,
            tipo_parq: formData.tipo_parq || '',
            titular: formData.titular || '',
            gestion: formData.gestion || '',
        ...
        fetch('/api/parks/', {
            ...
            .then(data => {
                ...
                Swal.fire({
                    ...
                    mapStore.listaParquesJardines.parques = true;
                    mapStore.listaParquesJardines.parquesocio = true;
                        ...
                        if (mapStore.showEquipmentForm) {
                            ...
                            fetch('/api/parks/', {
                                ...
                                .then(data => {
                                    mapStore.map.getSource('mi_parques').setData(data);
                                    try {
                                        mapStore.map.moveLayer('parques', 'mi_parques')
                                    } catch (error) {
                        ...
                        if (mapStore.showEquipmentForm) {
                            fetch('/api/parks/', {
                                ...
                                .then(data => {
                                    ...
                                    try {
                                        mapStore.map.moveLayer('parques', 'mi_parques')
                                        mapStore.map.moveLayer('inmigrantes', 'mi_parques')
                                    } catch (error) {
                                        console.log('error al mover capa', error)
                                    }
            ...
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <p v-if="!formData.nombre || formData.nombre.length > 50" class="text-red-500 text-xs mt-2">Campo
                        obligatorio y menor de 50 caracteres</p>
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <select v-model="formData.tipo_parq" id="tipo"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in parque_tipo_parq" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.tipo_parq" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="agua" class="block mb-2 text-sm font-medium text-gray-900 ">Agua</label>
                    <select v-model="formData.agua" id="agua"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                    ...
                    <p v-if="!formData.agua" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="saneamiento" class="block mb-2 text-sm font-medium text-gray-900 ">Saneamiento</label>
                    <select v-model="formData.saneamiento" id="saneamiento"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_sino" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                    ...
                    <p v-if="!formData.saneamiento" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="electricidad" class="block mb-2 text-sm font-medium text-gray-900 ">Electricidad</label>
                    <select v-model="formData.electricidad" id="electricidad"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_sino" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                    ...
                    <p v-if="!formData.electricidad" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="comedor" class="block mb-2 text-sm font-medium text-gray-900 ">Comedor</label>
                    <select v-model="formData.comedor" id="comedor"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_sino" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                    ...
                    <p v-if="!formData.comedor" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="juegos_inf" class="block mb-2 text-sm font-medium text-gray-900 ">Juegos
                        infantiles</label>
                    <select v-model="formData.juegos_inf" id="juegos_inf"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_sino" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                    ...
                    <p v-if="!formData.juegos_inf" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                    <label for="otras" class="block mb-2 text-sm font-medium text-gray-900 ">Otras</label>
                    <select v-model="formData.otras" id="otras"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in aa_sino" :key="estado.valor" :value="estado.valor">
                            {{ estado.descripcion }}
                    ...
                    <p v-if="!formData.otras" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>
                <div class="relative z-0 w-full mb-5 group">
