...
import { aa_estado, aa_acceso_s_ruedas, aa_fase, aa_gestion_equip, aa_titular_equip } from '@/composables/dominioComposable.js';
import { instal_deportiva_tipo_insde, inst_depor_deporte_tipo_depor } from '@/composables/sportsFacilityComposable.js';
...
const enable_nomecalles_layers = (layers) => {
    layers.forEach(layer => {
        mapStore.listaInstalacionesDeportivas[layer] = true
    })
}

const move_nomecalles_layers_behind_mine = (layers) => {
    layers.forEach(layer => {
        mapStore.map.moveLayer(layer, 'mi_instalaciones_deportivas')
    })
}
...
const getInfoByEquipmentId = async (gid) => {
    ...
    try {
        const data = await fetch(`/api/sports-facilities/${gid}`, {
            method: 'GET',
            headers: {
        ...
        formData.tipo_insde = result.tipo_insde
...
const deleteEquipment = (gid) => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            fetch(`/api/sports-facilities/${gid}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': 'Bearer ' + userStore.user.token,
                ...
                .then(response => {
                    ...
                    Swal.fire({
                        ...
                        fetch('/api/sports-facilities/', {
                            ...
                            .then(data => {
                                mapStore.map.getSource('mi_instalaciones_deportivas').setData(data);
...
const postInfo = () => {
    ...
    if (niveles_activos === 0) {
        alert('Debe seleccionar un deporte como mínimo.')
        return;
    }
    ...
    if (formData.gid) {
        ...
        const payload = {
            ...
            tipo_insde: formData.tipo_insde || '',
        ...
        fetch(`/api/sports-facilities/${formData.gid}`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': 'Bearer ' + userStore.user.token,
        ...
        const payload = {
            ...
            orden_instal: formData.orden_instal,
            nombre: formData.nombre,
            gestion: formData.gestion || '',
            titular: formData.titular || '',
            tipo_insde: formData.tipo_insde || '',
    ...
    <div
        ...
        <div class="flex justify-start">
            ...
            <p v-if="!formData.gid" class="font-bold text-xl md:text-2xl my-2">
                Alta equipamiento (Instalación deportiva)
            </p>
            <p v-if="formData.gid" class="font-bold text-xl md:text-2xl my-2 text-ellipsis">
                Editar equipamiento (Instalación deportiva)
            </p>
        </div>
    ...
    <div v-if="!isLoading">
        <form @submit.prevent="postInfo()" class="max-w-3xl mx-auto mt-28 select-none overscroll-none">
            ...
            <div class="grid md:grid-cols-1 md:gap-3">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    <label for="tipo_insde" class="block mb-2 text-sm font-medium text-gray-900 ">Tipo de instalación
                        deportiva</label>
                    <select v-model="formData.tipo_insde" id="tipo_insde"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 ">
                        <option selected value="">Elija una opción</option>
                        <option v-for="estado in instal_deportiva_tipo_insde " :key="estado.valor"
                            :value="estado.valor">{{
                estado.descripcion }}
                        </option>
                    </select>
                    <p v-if="!formData.tipo_insde" class="text-red-500 text-xs mt-2">Campo obligatorio</p>
                </div>

                <div class="relative z-0 w-full mb-5 group">
                    <p class="text-xl">Deportes</p>
                    <hr class="mb-4 mt-2">
                    <div class="grid grid-cols-1 gap-2">
                        <div v-for="nivel in inst_depor_deporte_tipo_depor" :key="nivel.valor">
